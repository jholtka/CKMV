...
//////-----------Template for RE model (Pitt & Lee 1981) ------------------
...
/*
K&L, p.102 eq(3.3.16)
1. half-n, 19/22-06-2015

2. trunc-n, 22-06-2015
*/

Sfa::fSfa(const vP, const prob, const avScore, const amHessian)
{
	decl epsi  = m_var[][0]-m_var[][1:m_cX]*vP[0:(m_cX-1)]; //NTx1
	...
	mustar = zeros(sizer(m_ID),1);
	sigstarsq = zeros(sizer(m_ID),1);
	sumsqeps = zeros(sizer(m_ID),1);
	epsbar = zeros(sizer(m_ID),1);
	lnL = zeros(sizer(m_ID),1);

if (m_iDist==TRUE) //truncated-normal			  
{	
	for (i = 0; i < sizer(m_ID); i++)//loopL[#firms]
	{
	veps = selectifr(epsi, m_vID .== i+1);//Tx1, eps_it

	sumsqeps[i][] = sumc(veps.^2);//SSR_i
	epsbar[i][] = meanc(veps);//bar_eps_i

	mustar[i][] = (mu*svsq -(m_Ti[i][] * susq * epsbar[i][])) / (svsq + m_Ti[i][]*susq);//tilde_mustar_i
	sigstarsq[i][] = (susq*svsq)/(svsq + m_Ti[i][]*susq);
	
	lnL[i][] = -(m_Ti[i][]/2)*log(2*M_PI)
	-(m_Ti[i][]-1)*log(sqrt(svsq)) - 0.5* log(svsq + m_Ti[i][]*susq)
	+ log(1- probn(-mustar[i][]/sqrt(sigstarsq[i][]))) -sumsqeps[i][]/(2*svsq) 
	+ 0.5* (mustar[i][]/sqrt(sigstarsq[i][]))^2
	- log(1- probn(-mu/sqrt(susq)))
	- 0.5* (mu/sqrt(susq))^2;
	}
}
else	//half-normal
{
	...
	mustar[i][] = -(m_Ti[i][] * susq * epsbar[i][]) / (svsq + m_Ti[i][]*susq);
	sigstarsq[i][] = (susq*svsq)/(svsq + m_Ti[i][]*susq);
	
	lnL[i][] = log(2) -(m_Ti[i][]/2)*log(2*M_PI)
	-(m_Ti[i][]-1)*log(sqrt(svsq)) - 0.5* log(svsq + m_Ti[i][]*susq)	
	+ log(1- probn(-mustar[i][]/sqrt(sigstarsq[i][]))) -sumsqeps[i][]/(2*svsq) 
	+ 0.5* (mustar[i][]/sqrt(sigstarsq[i][]))^2;	
	}
...
////	m_cX = columns(mx)+columns(mDum);
	...
	if (cU>1)	 //restrict 'cU' to 1
	{
		print("U_VAR: \"Constant\" only! \n");
        return FALSE;
	}
